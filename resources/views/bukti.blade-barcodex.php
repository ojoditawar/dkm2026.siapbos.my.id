...
<body>
    <div class="kuitansi-container">
        ...
        <div class="content">
            ...
            <!-- QR Code Section sebagai pengganti tanda tangan -->
            <div class="qr-section">
                <div class="qr-code">
                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=60x60&data={{ urlencode('KUITANSI-' . ($bukti->nomor ?? $bukti->kode) . '-' . $bukti->jumlah . '-' . $bukti->tanggal) }}"
                        alt="QR Code"
                        style="width: 58px; height: 58px;">
                </div>
                <div class="qr-info">
                    <div><strong>Verifikasi Digital</strong></div>
                    <!-- <div>Kuitansi No: {{ $bukti->nomor ?? $bukti->kode }}</div>
                    <div>Tanggal: {{ \Carbon\Carbon::parse($bukti->tanggal)->format('d/m/Y') }}</div>
                    <div>Jumlah: Rp {{ number_format($bukti->jumlah, 0, ',', '.') }}</div> -->
                </div>
            </div>

            <div class="verification-text">
                Scan QR Code untuk verifikasi keaslian kuitansi ini
            </div>
