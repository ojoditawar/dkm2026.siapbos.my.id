                    ...
                    ->schema([
                        ...
                        Select::make('dana')
                            ...
                            ->options(function ($get) {
                                return Level3::whereHas('level2', function ($query) {
                                    $query->where('akun1', '1');
                    ...
                    ->schema([
                        Select::make('anggaran_id')
                            ...
                            ->relationship(
                                ...
                                'uraian_detail',
                                fn($query, $get) => $query
                                    ->join('anggarans', 'anggaran_detail_items.anggaran_id', '=', 'anggarans.id')
                                    ->when($get('jenis_transaksi') === 'penerimaan', function ($query) use ($get) {
                                        ...
                                        return $query->where('anggarans.level1_id', 4); // Contoh: level1_id = 4 untuk penerimaan
                                    })
                                    ->when($get('jenis_transaksi') === 'pengeluaran', function ($query) use ($get) {
                                        // Filter berdasarkan jenis transaksi pengeluaran
                                        return $query->where('anggarans.level1_id', 5); // Contoh: level1_id = 5 untuk pengeluaran
                                    })
                                    ->select('anggaran_detail_items.*')
                            )
